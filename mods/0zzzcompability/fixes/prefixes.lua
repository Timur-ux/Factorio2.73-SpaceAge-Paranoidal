...
local function _FixRecipe(recipe)
	if recipe.name == "firearm-magazine" then
		log('!!! ' .. serpent.block(recipe, {maxlevel=3}))
		log('??? ' .. serpent.block(data.raw.item["bob-firearm-magazine"], {maxlevel=3}))
	end
...
local function FixRecipies()
	...
	for recipeName, recipe in pairs(data.raw.recipe) do
		if recipeName == "light-armod" then
			log(recipeName .. " " .. serpent.block(recipe, {maxlevel=2}))
		end
